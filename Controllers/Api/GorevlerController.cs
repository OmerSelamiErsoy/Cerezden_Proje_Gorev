...
public class GorevlerController : ControllerBase
{
    ...
    public async Task<ActionResult<KanbanViewModel>> Kanban([FromQuery] int? grupId, CancellationToken ct)
    {
        ...
        var iptalId = durumlar.FirstOrDefault(d => d.Ad == "İptal Edildi")?.Id ?? 0;
        var tamamlandiId = durumlar.FirstOrDefault(d => d.Ad == "Tamamlandı")?.Id ?? 0;
        ...
        if (!genelYetkiliMi)
        {
            ...
            tumIds = gorevIds.Union(atananIds).Union(sorumluIds).Distinct().ToList();
        }

        var aktifGorevlerQuery = _db.Gorevler
            .AsNoTracking()
            .Include(g => g.Durum)
        ...
        var tamamlananIptalQuery = _db.Gorevler
            .AsNoTracking()
            .Include(g => g.Durum)
            .Include(g => g.OlusturanKullanici)
            .Include(g => g.SorumluKullanici)
        ...
        List<Gorev> aktifGorevler;
        List<Gorev> tamamlananIptal;

        if (genelYetkiliMi)
        {
            aktifGorevler = await aktifGorevlerQuery
                .Where(g => g.DurumId != iptalId && g.DurumId != tamamlandiId && (grupId == null || g.GorevGrubuId == grupId))
                .ToListAsync(ct);

            tamamlananIptal = await tamamlananIptalQuery
                .Where(g => (g.DurumId == iptalId || g.DurumId == tamamlandiId) && (grupId == null || g.GorevGrubuId == grupId))
                .ToListAsync(ct);
        }
        else
        {
            aktifGorevler = await aktifGorevlerQuery
                .Where(g => tumIds.Contains(g.Id) && g.DurumId != iptalId && g.DurumId != tamamlandiId && (grupId == null || g.GorevGrubuId == grupId))
                .ToListAsync(ct);
    ...
    public async Task<ActionResult<List<GorevDurumOzetDto>>> DurumOzet([FromQuery] bool sorumluOldugum = false, CancellationToken ct = default)
    {
        ...
        // Genel yetkili ise tüm görevleri say.
        if (genelYetkiliMi)
        {
            var toplamSayilar = await _db.Gorevler
                .AsNoTracking()
                .Where(g => !g.IsDeleted && durumIdler.Contains(g.DurumId))
                .GroupBy(g => g.DurumId)
                .Select(g => new { DurumId = g.Key, Sayi = g.Count() })
                .ToListAsync(ct);
        ...
        var toplamSayilar2 = await _db.Gorevler
            .AsNoTracking()
            .Where(g => !g.IsDeleted
                && durumIdler.Contains(g.DurumId)
        ...
        var toplamSonuc2 = durumlar
            .Select(d => new GorevDurumOzetDto
            {
                DurumAd = d.Ad,
                Adet = toplamSayilar2.FirstOrDefault(x => x.DurumId == d.Id)?.Sayi ?? 0
            })
            .OrderBy(x => Array.IndexOf(hedefDurumAdlari, x.DurumAd))
            .ToList();

        return Ok(toplamSonuc2);
    }
    ...
    public async Task<IActionResult> Update(int id, [FromBody] GorevUpdateDto dto, CancellationToken ct)
    {
        ...
        // Tür / sorumlu / atamalar sadece gerçekten payload'da gelmişse güncellenmeli.
        // Aksi halde (ör. kanban'dan sadece durum güncellemesi) paylaşılan kişiler siliniyordu.
        if (dto.Turu.HasValue)
            gorev.Turu = dto.Turu.Value;

        if (dto.SorumluKullaniciId.HasValue)
            gorev.SorumluKullaniciId = dto.SorumluKullaniciId.Value;
        else if (dto.Turu is 0)
            gorev.SorumluKullaniciId = userId;
        ...
        if (dto.AtananKullaniciIds != null)
        {
            ...
            foreach (var kid in dto.AtananKullaniciIds.Distinct())
